//! Vector index management for MotionDB.
//!
//! Manages per-table HNSW indexes for VECTOR columns, bridging the
//! `motion-hnsw` crate into the database engine layer.

use std::collections::HashMap;
use std::sync::Arc;

use motion_hnsw::{DistanceMetric, HnswConfig, HnswIndex, HnswResult, SearchResult, VectorId};
...
impl VectorIndexManager {
    ...
    pub fn create_index(
        ...
        if indexes.contains_key(&key) {
            return Err(motion_hnsw::HnswError::InvalidConfig(format!(
    ...
    pub fn insert(
        ...
        let index = indexes
            .get(key)
            .ok_or(motion_hnsw::HnswError::InvalidConfig(format!(
