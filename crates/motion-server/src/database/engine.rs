...
//!
//! The `Database` struct is the top-level entry point for MotionDB.
...
use motion_security::{AuditLog, Authenticator, Authorizer};
use motion_sql::storage::StorageEngine;
use motion_wal::{SyncPolicy, Wal, WalConfig};
...
pub const DEFAULT_DATABASE_NAME: &str = "motiondb";
...
impl Database {
    ...
    fn replay_wal(wal: &Wal, _storage: &StorageEngine) -> DatabaseResult<()> {
        use motion_wal::record::RecordType;
    ...
    pub fn create_authenticated_session(
        self: &Arc<Self>,
        database_name: &str,
        identity: motion_security::Identity,
    ) -> SessionId {
        let id = SessionId::new(self.next_session_id.fetch_add(1, Ordering::SeqCst));
...
mod tests {
    ...
    fn make_test_credential(username: &str, label: &str) -> motion_security::Credential {
        motion_security::Credential::Password {
    ...
    fn test_security_enforced_auth_blocks_unauthorized() {
        ...
        // Create the reader role with only SELECT on motiondb.*
        db.authorizer()
            .create_role(motion_security::Role::new("reader").with_permission(
                motion_security::Permission::Database {
                    database: DEFAULT_DATABASE_NAME.to_string(),
                    privilege: motion_security::Privilege::Select,
                },
            ))
            .unwrap();
    ...
    fn test_security_audit_log_captures_auth_events() {
        ...
        let bad_cred = motion_security::Credential::Password {
    ...
    fn test_security_rbac_select_allowed_insert_denied() {
        ...
        db.authenticator()
            ...
            .unwrap();
        db.authorizer()
            .create_role(motion_security::Role::new("viewer").with_permission(
                motion_security::Permission::Table {
                    database: DEFAULT_DATABASE_NAME.to_string(),
    ...
    fn test_security_alter_table_authorization_denied() {
        ...
        db.authenticator()
            ...
            .unwrap();
        db.authorizer()
            .create_role(motion_security::Role::new("selector").with_permission(
                motion_security::Permission::Table {
                    database: DEFAULT_DATABASE_NAME.to_string(),
