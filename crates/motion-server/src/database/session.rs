...
use motion_cache::plan_cache::{PlanCache, PlanCacheConfig};
use motion_cache::result_cache::{ResultCache, ResultCacheConfig, ResultCacheKey};
use motion_common::types::TxnId;
use motion_mvcc::IsolationLevel;
use motion_security::audit::{AuditAction, AuditLog};
use motion_security::authn::Identity;
use motion_security::authz::{Authorizer, Privilege};
use motion_sql::executor::{Row, Value};
use motion_sql::logical::{build_plan, Field, MemoryCatalog, Schema};
use motion_sql::optimizer::{Optimizer, OptimizerConfig};
use motion_sql::parser::{
    BinaryOperator, ColumnConstraint, Expr, InsertSource, Literal, Parser, Statement, UnaryOperator,
};
use motion_sql::physical::{ExecutionContext, PhysicalPlan, PhysicalPlanner};
use motion_sql::storage::{StorageEngine, TableInfo};
...
impl Session {
    ...
    fn handle_sequence_function(
        ...
        use motion_sql::executor::{RecordBatch, Row, Value};
        use motion_sql::logical::{Field, Schema};
        use motion_sql::parser::DataType;
    ...
    fn build_returning_result(
        &self,
        rows: &[Row],
        table_info: &motion_sql::storage::TableInfo,
        returning: &[motion_sql::parser::SelectItem],
    ) -> DatabaseResult<StatementResult> {
        use motion_sql::executor::RecordBatch;
        use motion_sql::logical::{Field, Schema};
        ...
        for item in returning {
            match &item.expr {
                motion_sql::parser::Expr::Wildcard => {
                ...
                motion_sql::parser::Expr::Column(cr) => {
    ...
    pub fn dump_database(&self) -> DatabaseResult<String> {
        ...
        output.push_str("-- MotionDB database dump\n");
    ...
    fn generate_create_table(&self, name: &str, info: &motion_sql::storage::TableInfo) -> String {
    ...
    fn execute_virtual_schema(
        ...
        let schema_ref = std::sync::Arc::new(motion_sql::logical::Schema::empty());
    ...
    fn server_version_result(&self) -> StatementResult {
        ...
        let row = Row::new(vec![Value::String(
            "MotionDB 0.1.0 (PostgreSQL compatible)".to_string(),
        )]);
        let schema_ref = std::sync::Arc::new(schema);
        let batch = RecordBatch::from_rows(schema_ref.clone(), &[row])
        ...
        let decision = match statement {
            ...
            Statement::Select(select) => {
                ...
                if tables.is_empty() && !select.from.is_empty() {
                    ...
                    motion_security::AccessDecision::Allow
                }
            }
            ...
            | Statement::UseDatabase(_) => motion_security::AccessDecision::Allow,
            ...
            _ => motion_security::AccessDecision::Deny("statement type not authorized".to_string()),
        };

        match decision {
            motion_security::AccessDecision::Allow => Ok(()),
            motion_security::AccessDecision::Deny(reason) => {
    ...
    fn execute_create_table(
        &self,
        create: &motion_sql::parser::CreateTableStatement,
    ) -> DatabaseResult<StatementResult> {
        ...
        if self.storage().table_exists(name) {
            ...
            return Err(DatabaseError::StorageError(
                motion_sql::storage::StorageError::TableExists(name.clone()),
            ));
        }
        ...
        if primary_key.is_empty() {
            for tc in &create.constraints {
                if let motion_sql::parser::TableConstraint::PrimaryKey { columns, .. } = tc {
                    primary_key = columns
                        .iter()
        ...
        for tc in &create.constraints {
            if let motion_sql::parser::TableConstraint::Unique { columns, .. } = tc {
        ...
        let defaults: Vec<motion_sql::storage::ColumnDefault> = create
            .columns
            .iter()
            .enumerate()
            .filter_map(|(i, col)| {
                col.default.as_ref().and_then(|expr| {
                    self.eval_literal(expr)
                        .ok()
                        .map(|val| motion_sql::storage::ColumnDefault {
                            col_idx: i,
                            value: val,
                        })
                })
        ...
        let mut check_constraints: Vec<motion_sql::storage::CheckConstraint> = create
            .columns
            .iter()
            .filter_map(|col| {
                col.constraints.iter().find_map(|c| {
                    if let ColumnConstraint::Check(expr) = c {
                        Some(motion_sql::storage::CheckConstraint {
                            expr: expr.clone(),
                            sql: format!("{:?}", expr),
                        })
                    } else {
            ...
            .collect();
        for tc in &create.constraints {
            if let motion_sql::parser::TableConstraint::Check { expr, .. } = tc {
                check_constraints.push(motion_sql::storage::CheckConstraint {
                    expr: expr.clone(),
                    sql: format!("{:?}", expr),
                });
            }
        ...
        for tc in &create.constraints {
            if let motion_sql::parser::TableConstraint::ForeignKey {
                columns: fk_cols,
                ref_table,
            ...
            } = tc
            {
                let col_indices: Vec<usize> = fk_cols
                    ...
                    .collect();
                foreign_keys.push(motion_sql::storage::ForeignKeyConstraint {
                    columns: col_indices,
        ...
        for (i, col) in create.columns.iter().enumerate() {
            for constraint in &col.constraints {
                if let motion_sql::parser::ColumnConstraint::References {
                    table,
                    column: ref_col,
                } = constraint
                {
    ...
    fn execute_drop_table(
        &self,
        drop: &motion_sql::parser::DropTableStatement,
    ) -> DatabaseResult<StatementResult> {
        for table_ref in &drop.names {
            ...
            if !self.storage().table_exists(name) {
                ...
                return Err(DatabaseError::StorageError(
                    motion_sql::storage::StorageError::TableNotFound(name.clone()),
                ));
            }
    ...
    fn execute_alter_table(
        &self,
        alter: &motion_sql::parser::AlterTableStatement,
    ) -> DatabaseResult<StatementResult> {
        use motion_sql::parser::AlterOperation;
        ...
        if !self.storage().table_exists(table_name) {
            ...
            return Err(DatabaseError::StorageError(
                motion_sql::storage::StorageError::TableNotFound(table_name.clone()),
            ));
        }
        ...
        for op in &alter.operations {
            let table_info = self.storage().get_table_info(table_name).ok_or_else(|| {
                DatabaseError::StorageError(motion_sql::storage::StorageError::TableNotFound(
                    table_name.clone(),
                ))
            })?;
            ...
            match op {
                ...
                AlterOperation::DropColumn { name, if_exists } => {
                    ...
                    new_info.defaults = table_info
                        ...
                        .map(|d| motion_sql::storage::ColumnDefault {
    ...
    fn execute_create_index(
        &self,
        create: &motion_sql::parser::CreateIndexStatement,
    ) -> DatabaseResult<StatementResult> {
        use motion_sql::parser::DataType;
        ...
        if is_vector_index {
            ...
            let metric =
                super::vector_index::parse_metric("l2").unwrap_or(motion_hnsw::DistanceMetric::L2);
            let m = 16;
            ...
            // Register in catalog
            let index_info = motion_sql::storage::IndexInfo::hnsw(
                &create.name,
                col_idx,
                "l2",
                m,
            ...
            let index_info =
                motion_sql::storage::IndexInfo::new(&create.name, col_indices, create.unique);
            self.storage().catalog().add_index(table_name, index_info)?;
    ...
    fn execute_drop_index(
        &self,
        drop: &motion_sql::parser::DropIndexStatement,
    ) -> DatabaseResult<StatementResult> {
    ...
    fn execute_insert(
        &mut self,
        insert: &motion_sql::parser::InsertStatement,
    ) -> DatabaseResult<StatementResult> {
                    ...
                    for (i, expr) in value_row.iter().enumerate() {
                        if i < column_order.len() {
                            ...
                            let val = if matches!(
                                ...
                                motion_sql::parser::DataType::Vector(_)
                            ) {
                            ...
                            if let Value::Vector(ref vec_data) = val {
                                if let motion_sql::parser::DataType::Vector(expected_dim) =
                                    &schema.fields()[col_idx].data_type
                                {
        ...
        let count = if on_conflict.is_some() {
            ...
            for row in &rows {
                match self.storage().execute_insert(table_name, vec![row.clone()]) {
                    Ok(n) => inserted += n,
                    Err(motion_sql::storage::StorageError::PrimaryKeyViolation(_)) => {
                        match on_conflict.as_ref().unwrap().action {
                            motion_sql::parser::ConflictAction::DoNothing => {
                                // Skip this row silently
                                continue;
                            }
                            motion_sql::parser::ConflictAction::DoUpdate(ref assignments) => {
    ...
    fn execute_update(
        &mut self,
        update: &motion_sql::parser::UpdateStatement,
    ) -> DatabaseResult<StatementResult> {
    ...
    fn execute_delete(
        &mut self,
        delete: &motion_sql::parser::DeleteStatement,
    ) -> DatabaseResult<StatementResult> {
    ...
    fn execute_select_for_insert(&self, statement: &Statement) -> DatabaseResult<Vec<Row>> {
        ...
        for tbl in self.storage().list_tables() {
            if let Some(info) = self.storage().get_table_info(&tbl) {
                let mut meta =
                    motion_sql::logical::TableMeta::new(tbl.clone(), (*info.schema).clone());
                ...
                for idx in &info.indexes {
                    let col_names: Vec<String> = idx
                        ...
                        .collect();
                    meta.indexes.push(motion_sql::logical::IndexMeta::new(
        ...
        let mut executor = motion_sql::executor::QueryExecutor::new(exec_ctx);
    ...
    fn execute_query(
        ...
        let physical_plan = if let Some(cached_plan) = self.plan_cache.get(sql) {
            ...
            for table_name in self.storage().list_tables() {
                if let Some(table_info) = self.storage().get_table_info(&table_name) {
                    let mut meta = motion_sql::logical::TableMeta::new(
                        table_name.clone(),
        ...
        // ── Execute ─────────────────────────────────────────────
        let ctx = ExecutionContext::default();
        let mut executor = motion_sql::executor::QueryExecutor::new(ctx);
    ...
    fn execute_subquery_select(
        &self,
        subquery: &motion_sql::parser::SelectStatement,
    ) -> DatabaseResult<Vec<Row>> {
        let stmt = Statement::Select(subquery.clone());
        let mut catalog = MemoryCatalog::new();
        for table_name in self.storage().list_tables() {
            if let Some(table_info) = self.storage().get_table_info(&table_name) {
                let meta = motion_sql::logical::TableMeta::new(
                    table_name.clone(),
                    (*table_info.schema).clone(),
                );
                catalog.add_table(meta);
    ...
    fn execute_recursive_cte(
        &self,
        cte: &motion_sql::parser::CTE,
    ) -> DatabaseResult<Vec<motion_sql::executor::RecordBatch>> {
        ...
        for _ in 0..MAX_ITERATIONS {
            ...
            if let Some(first_batch) = all_batches.first() {
                ...
                let cte_schema = motion_sql::logical::Schema::new(schema_ref.fields().to_vec());
                let meta = motion_sql::logical::TableMeta::new(cte.name.clone(), cte_schema);
                catalog.add_table(meta);
            }
    ...
    fn execute_cte_query(
        &self,
        cte_select: &motion_sql::parser::SelectStatement,
    ) -> DatabaseResult<Vec<motion_sql::executor::RecordBatch>> {
    ...
    fn execute_explain(
        &self,
        inner: &Statement,
        format: &motion_sql::parser::ExplainFormat,
        analyze: bool,
        start: Instant,
    ) -> DatabaseResult<StatementResult> {
        use motion_sql::parser::ExplainFormat;
        ...
        let stats = motion_sql::optimizer::Statistics::default();
        ...
        let schema = Arc::new(Schema::new(vec![Field::not_null(
            "plan",
            motion_sql::parser::DataType::Text,
        )]));
        ...
        let batch = motion_sql::executor::RecordBatch::from_rows(schema.clone(), &rows)
            .map_err(|e| DatabaseError::ExecutionError(e))?;
    ...
    fn analyze_plan_issues(&self, plan: &PhysicalPlan) -> (Vec<String>, Vec<String>) {
        ...
        let seq_scans = plan
            .find_operators(|op| matches!(op, motion_sql::physical::PhysicalOperator::SeqScan(_)));
        for op in &seq_scans {
            if let motion_sql::physical::PhysicalOperator::SeqScan(scan) = op {
        ...
        let nl_joins = plan.find_operators(|op| {
            matches!(op, motion_sql::physical::PhysicalOperator::NestedLoopJoin(_))
        });
        for op in &nl_joins {
            if let motion_sql::physical::PhysicalOperator::NestedLoopJoin(join) = op {
    ...
    fn collect_from_items(from: &[motion_sql::parser::FromItem], tables: &mut Vec<String>) {
        for item in from {
            match item {
                motion_sql::parser::FromItem::Table(t) => {
                    tables.push(t.table.clone());
                }
                motion_sql::parser::FromItem::Join { left, right, .. } => {
                    Self::collect_from_items(&[*left.clone()], tables);
                    Self::collect_from_items(&[*right.clone()], tables);
                }
                motion_sql::parser::FromItem::Subquery { query, .. } => {
    ...
    fn apply_defaults(row: &mut Row, table_info: &motion_sql::storage::TableInfo) {
    ...
    fn derive_vector_id(row: &Row, table_info: &motion_sql::storage::TableInfo) -> u64 {
    ...
    fn eval_expr(&self, row: &Row, expr: &Expr, schema: &Schema) -> DatabaseResult<Value> {
        match expr {
            ...
            Expr::Function(func) => {
                ...
                motion_sql::executor::evaluate_scalar_function(&func.name, &args)
...
mod tests {
    ...
    fn test_session_show_databases() {
        ...
        if let StatementResult::Query(query_result) = result {
            ...
            let rows = query_result.rows();
            assert_eq!(rows[0].get(0), Some(&Value::String("motiondb".to_string())));
        } else {
            panic!("expected Query result");
        }
    }
    ...
    fn test_session_explain_json() {
        ...
        let inner = motion_sql::parser::Parser::parse_one("SELECT * FROM users").unwrap();
        let format = motion_sql::parser::ExplainFormat::Json;
