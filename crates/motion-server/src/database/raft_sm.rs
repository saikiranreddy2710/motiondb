//! Raft StateMachine backed by MotionDB's StorageEngine.
//!
//! Implements `motion_raft::StateMachine` to apply committed log entries
...
//! ```text
//! { "db": "motiondb", "sql": "INSERT INTO users VALUES (1, 'Alice')" }
//! ```
...
use motion_raft::state_machine::{ApplyResult, Snapshot, StateMachine};
use motion_raft::LogIndex;
...
/// Raft StateMachine that applies SQL commands to a MotionDB Database.
///
...
impl StateMachine for DatabaseStateMachine {
    ...
    fn apply(&mut self, entry: &motion_raft::rpc::LogEntry) -> ApplyResult {
...
mod tests {
    ...
    fn test_raft_command_roundtrip() {
        let cmd = RaftCommand::new("motiondb", "INSERT INTO users VALUES (1, 'Alice')");
        let bytes = cmd.to_bytes();
        let decoded = RaftCommand::from_bytes(&bytes).unwrap();
        assert_eq!(decoded.db, "motiondb");
