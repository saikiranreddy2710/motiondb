//! MotionDB Server Daemon
//!
//! The `motiond` binary is the main MotionDB server process that:
...
//! motiond --data-dir /var/lib/motiondb
//!
//! # Start on custom port
//! motiond --port 5433
//!
//! # Use configuration file
//! motiond --config /etc/motiondb/motiond.toml
//! ```
...
use motion_server::config::ServerConfig;
use motion_server::database::{Database, DatabaseConfig};
use motion_server::grpc::GrpcServer;
...
#[command(
    ...
    version,
    about = "MotionDB database server",
    long_about = "MotionDB is a high-performance NewSQL database.\n\n\
                  This daemon starts the database server and listens for client connections."
)]
struct Args {
    /// Host address to bind to
    #[arg(short = 'H', long, default_value = "0.0.0.0", env = "MOTION_HOST")]
    host: String,
    ...
    #[arg(short = 'd', long, value_name = "DIR", env = "MOTION_DATA_DIR")]
    ...
    #[arg(long, default_value = "info", env = "MOTION_LOG_LEVEL")]
    log_level: String,

    /// Maximum number of connections
    #[arg(long, default_value_t = 100, env = "MOTION_MAX_CONNECTIONS")]
    max_connections: usize,

    /// Buffer pool size in MB
    #[arg(long, default_value_t = 128, env = "MOTION_BUFFER_POOL_MB")]
    buffer_pool_mb: usize,

    /// WAL directory (defaults to data_dir/wal)
    #[arg(long, value_name = "DIR", env = "MOTION_WAL_DIR")]
    wal_dir: Option<PathBuf>,

    /// Enable authentication (creates default admin user on first start)
    #[arg(long, env = "MOTION_AUTH")]
    auth: bool,

    /// Admin username (used with --auth for bootstrap)
    #[arg(long, default_value = "admin", env = "MOTION_ADMIN_USER")]
    admin_user: String,

    /// Admin password (used with --auth for bootstrap; required when --auth is set)
    #[arg(long, env = "MOTION_ADMIN_PASSWORD")]
    admin_password: Option<String>,

    /// Enable Raft distributed consensus
    #[arg(long, env = "MOTION_RAFT")]
    raft: bool,

    /// Raft node ID (unique per cluster member)
    #[arg(long, default_value_t = 1, env = "MOTION_NODE_ID")]
    node_id: u64,

    /// Raft peer addresses (comma-separated: "2=host2:7001,3=host3:7001")
    #[arg(long, env = "MOTION_RAFT_PEERS", value_delimiter = ',')]
...
fn init_logging(args: &Args) {
    ...
    let filter = EnvFilter::try_new(format!(
        "motion_server={level},motion_sql={level},motion_storage={level},motion_txn={level}"
    ))
...
fn print_banner() {
    ...
    info!("║   ╚═╝  ╚═══╝╚══════╝╚═╝  ╚═╝ ╚═════╝ ╚══════╝         ║");
    info!("║                                                       ║");
    info!("║          MotionDB v{:<10} - NewSQL Database        ║", version);
    info!("║                                                       ║");
    info!("╚═══════════════════════════════════════════════════════╝");
}
...
async fn run_server(config: ServerConfig) -> Result<()> {
            ...
            _ => {
                ...
                warn!(
                    "No admin password provided — generated random password. \
                     Set MOTION_ADMIN_PASSWORD for production!"
                );
    ...
    if config.enable_raft {
        let raft_config = motion_raft::RaftConfig::new(config.node_id)
            .with_peers(
                config
                    .raft_peers
                    .iter()
        ...
        let state_machine =
            motion_server::database::DatabaseStateMachine::new(db.clone());
        let raft_node = motion_raft::RaftNode::new(raft_config, state_machine);
    ...
    let pg_factory = Arc::new(motion_server::pgwire_handler::MotionPgWireFactory::new(
        db.clone(),
    ));
...
async fn run_mysql_server(addr: SocketAddr, db: Arc<Database>) -> Result<()> {
    ...
    loop {
        ...
        let handler = motion_server::mysql_handler::MysqlQueryHandler::new(db.clone());
...
async fn run_pgwire_server(
    addr: SocketAddr,
    factory: Arc<motion_server::pgwire_handler::MotionPgWireFactory>,
) -> Result<()> {
    let listener = tokio::net::TcpListener::bind(addr).await?;
    loop {
