...
//!
//! This module provides configuration management for the MotionDB server.
...
pub struct ServerConfig {
    ...
    /// Use MOTION_ADMIN_PASSWORD env var for security.
    #[serde(default)]
...
mod tests {
    ...
    fn test_effective_wal_dir() {
        let config = ServerConfig::builder().data_dir("/data/motion").build();

        assert_eq!(
            config.effective_wal_dir(),
            Some(PathBuf::from("/data/motion/wal"))
        );

        let config2 = ServerConfig::builder().data_dir("/data/motion").build();
