//! gRPC service implementation for MotionDB.
//!
//! This module implements the MotionDB gRPC service defined in motion-proto.
...
use motion_proto::proto::{
    motion_db_server::MotionDb, BeginTransactionRequest, BeginTransactionResponse, ColumnInfo,
...
use motion_security::{Credential, Identity};
...
/// MotionDB gRPC service implementation.
pub struct MotionDbService {
...
impl MotionDbService {
    ...
    /// - `x-motion-username` + `x-motion-password`: password auth
    ///
    ...
    fn authenticate<T>(&self, request: &Request<T>) -> Result<Identity, Status> {
        ...
        if let (Some(username), Some(password)) = (
            metadata.get("x-motion-username"),
            metadata.get("x-motion-password"),
        ) {
        ...
        if self.db.authenticator().is_enforcing() {
            Err(Status::unauthenticated(
                "authentication required; provide 'authorization' header or 'x-motion-username'/'x-motion-password' metadata",
            ))
        } else {
        ...
        let proto_rows: Vec<Row> = result
            ...
            .map(|row| {
                let values: Vec<Value> = row
                    .iter()
                    .map(|v| {
                        use motion_proto::proto::value::Value as ProtoValue;
                        Value {
    ...
    fn statement_to_response(
        ...
        use motion_proto::proto::execute_response::Result;
...
#[tonic::async_trait]
impl MotionDb for MotionDbService {
    ...
    async fn get_server_info(
        ...
        Ok(Response::new(ServerInfoResponse {
            ...
            server_name: "MotionDB".to_string(),
            protocol_version: 1,
            features: vec![
                "SQL".to_string(),
...
impl GrpcServer {
    ...
    pub async fn serve(self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let service = MotionDbService::new(self.db);
        ...
        match self.tls_config {
            Some(tls) => {
                ...
                if let Some(ca_cert) = &tls.client_ca_cert {
                    ...
                    tracing::info!(
                        "MotionDB gRPC server listening on {} (mTLS enabled)",
                        self.addr
                    );
                } else {
                    tracing::info!(
                        "MotionDB gRPC server listening on {} (TLS enabled)",
                        self.addr
                    );
                }
                ...
                tonic::transport::Server::builder()
                    ...
                    .add_service(motion_proto::MotionDbServer::new(service))
                    .serve(self.addr)
                    .await?;
            }
            None => {
                tracing::info!(
                    "MotionDB gRPC server listening on {} (plaintext)",
                    self.addr
                );
...
mod tests {
    ...
    fn test_service_creation() {
        ...
        let _service = MotionDbService::new(Arc::new(db));
    }
}
