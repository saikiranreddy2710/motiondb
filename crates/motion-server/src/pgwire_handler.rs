//! PostgreSQL wire protocol handler for MotionDB.
//!
...
/// Maps MotionDB `DataType` to PostgreSQL `Type` OID.
...
/// MotionDB query handler for PostgreSQL wire protocol.
...
impl PgWireQueryHandler {
    ...
    fn execute_one(&self, sql: &str) -> PgWireResult<Response<'_>> {
        ...
        let session_id = self.db.create_session("motiondb");
    ...
    /// Converts a MotionDB StatementResult to a pgwire Response.
    ...
    /// Encodes a MotionDB Value into a pgwire column.
...
pub struct MotionPgWireFactory {
    handler: Arc<PgWireQueryHandler>,
}

impl MotionPgWireFactory {
    pub fn new(db: Arc<Database>) -> Self {
        Self {
...
impl PgWireHandlerFactory for MotionPgWireFactory {
