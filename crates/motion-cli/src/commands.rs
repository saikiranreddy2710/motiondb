                ...
                if let Some(client) = repl.client() {
                    ...
                    match client.execute("SHOW TABLES").await {
                        Ok(result) => {
                            ...
                            dump.push_str("-- MotionDB SQL Dump\n\n");

                            // Get table list from result
                            for row in &result.rows {
                                if let Some(motion_client::Value::String(table_name)) =
                                    row.first()
                                {
                                        ...
                                        .await
                                    {
                                        for row in &data.rows {
                                            let vals: Vec<String> = row
                                                .iter()
                                                .map(|v| match v {
                                                    motion_client::Value::Null => {
                                                        "NULL".to_string()
                                                    }
                                                    motion_client::Value::String(s) => {
                                                        format!(
                                                            "'{}'",
                                                            s.replace('\'', "''")
                                                        )
