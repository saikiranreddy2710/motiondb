    ...
    for (row_idx, row) in result.rows.iter().enumerate() {
        ...
        for (col_idx, value) in row.iter().enumerate() {
            ...
            let display = match value {
                motion_client::Value::Null => "(null)".to_string(),
                v => v.to_string(),
            };
...
fn value_to_json(value: &motion_client::Value) -> JsonValue {
    match value {
        motion_client::Value::Null => JsonValue::Null,
        motion_client::Value::Boolean(b) => json!(*b),
        motion_client::Value::Integer(i) => json!(*i),
        motion_client::Value::Float(f) => json!(*f),
        motion_client::Value::String(s) => json!(s),
        motion_client::Value::Bytes(b) => {
...
mod tests {
    ...
    fn make_test_result() -> QueryResult {
        QueryResult {
            ...
            rows: vec![
                vec![
                    motion_client::Value::Integer(1),
                    motion_client::Value::String("Alice".to_string()),
                    motion_client::Value::Boolean(true),
                ],
                vec![
                    motion_client::Value::Integer(2),
                    motion_client::Value::String("Bob".to_string()),
                    motion_client::Value::Boolean(false),
                ],
            ],
            rows_affected: 0,
