...
impl CliConfig {
    ...
    pub fn connection_info(&self) -> ConnectionInfo {
        ConnectionInfo {
            ...
            port: self.port,
            database: self.database.clone().unwrap_or_else(|| "motiondb".into()),
        }
    }
    ...
    /// 1. ~/.config/motiondb/config.toml
    /// 2. ~/.motiondb/config.toml
    /// 3. Returns default if not found
    pub async fn load_default() -> Result<Self> {
        // Try XDG config first
        if let Some(config_dir) = dirs::config_dir() {
            let path = config_dir.join("motiondb").join("config.toml");
        ...
        if let Some(home) = dirs::home_dir() {
            let path = home.join(".motiondb").join("config.toml");
            if tokio::fs::try_exists(&path).await.unwrap_or(false) {
                return Self::from_file(&path).await;
            }
        }
    ...
    pub fn default_config_path() -> Option<PathBuf> {
        dirs::config_dir().map(|d| d.join("motiondb").join("config.toml"))
    }
