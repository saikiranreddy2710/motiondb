//! Interactive REPL (Read-Eval-Print-Loop) for MotionDB.
//!
...
use motion_client::{Client, ClientConfig, QueryResult};
...
impl Repl {
    ...
    pub fn print_banner(&self) {
        println!("MotionDB CLI v{}", env!("CARGO_PKG_VERSION"));
    ...
    pub async fn connect(&mut self) -> Result<()> {
        if self.config.mock_mode {
            ...
            println!("Connected to MotionDB (mock mode — no data is persisted)");
            return Ok(());
        }
    ...
    fn get_prompt(&self) -> String {
        let db = self.config.database.as_deref().unwrap_or("motiondb");
        let mock = if self.mock_mode { "[mock]" } else { "" };
        if self.in_transaction {
            format!("motion({}){}* > ", db, mock)
        } else {
            format!("motion({}){}> ", db, mock)
        }
    }
...
fn get_history_file() -> Option<std::path::PathBuf> {
    dirs::data_local_dir().map(|dir| dir.join("motiondb").join("history"))
}
