...
/// The main error type for MotionDB.
///
...
/// use motion_common::types::PageId;
///
/// fn read_page(page_id: PageId) -> MotionResult<Vec<u8>> {
///     Err(MotionError::PageNotFound { page_id })
/// }
/// ```
#[derive(Debug, Error)]
...
mod tests {
    ...
    #[test]
    fn test_error_code() {
        let err = MotionError::PageNotFound {
    ...
    #[test]
    fn test_retryable() {
        assert!(MotionError::BufferPoolFull.is_retryable());
        assert!(MotionError::LeaderUnknown.is_retryable());
        assert!(!MotionError::KeyNotFound.is_retryable());
    }

    #[test]
    fn test_conflict() {
        let err = MotionError::TransactionConflict {
            txn_id: TxnId::new(1),
            conflicting_txn_id: TxnId::new(2),
        };
        assert!(err.is_conflict());
        assert!(!MotionError::KeyNotFound.is_conflict());
    }

    #[test]
    fn test_io_error_from() {
        let io_err = std::io::Error::new(std::io::ErrorKind::NotFound, "file not found");
        let motion_err: MotionError = io_err.into();
        assert_eq!(motion_err.code(), ErrorCode::Io);
    }
}
