...
//!
//! Provides the main `Client` struct for connecting to MotionDB and executing queries.
...
use motion_proto::proto::{
    motion_db_client::MotionDbClient, BeginTransactionRequest, CommitRequest, ExecuteRequest,
...
impl Default for ClientConfig {
    fn default() -> Self {
        Self {
            ...
            username: None,
            password: None,
            application_name: Some("motion-client".to_string()),
            auto_reconnect: true,
...
struct ConnectionInner {
    ...
    grpc_client: Option<MotionDbClient<Channel>>,
}
...
impl Client {
            ...
            match &inner.grpc_client {
                ...
                None => {
                    ...
                    return Ok(ServerInfo {
                        ...
                        server_name: "MotionDB (mock)".to_string(),
                        protocol_version: 1,
    ...
    pub async fn connect(&self) -> ClientResult<()> {
        ...
        let connect_result = tokio::time::timeout(
            ...
            MotionDbClient::connect(endpoint.clone()),
        )
        .await;
    ...
    ///
    /// Injects `x-motion-username` and `x-motion-password` headers from
    ...
    fn attach_auth<T>(&self, request: &mut tonic::Request<T>) {
        if let Some(ref user) = self.config.username {
            if let Ok(val) = user.parse() {
                request.metadata_mut().insert("x-motion-username", val);
            }
        }
        if let Some(ref pass) = self.config.password {
            if let Ok(val) = pass.parse() {
                request.metadata_mut().insert("x-motion-password", val);
            }
        }
    }
    ...
    async fn execute_query_internal(
        ...
        match response.result {
            Some(motion_proto::proto::execute_response::Result::QueryResult(qr)) => {
            ...
            Some(motion_proto::proto::execute_response::Result::CommandResult(cr)) => {
                Ok(QueryResult {
                    columns: vec![],
                    rows: vec![],
...
fn proto_value_to_value(v: &motion_proto::proto::Value) -> Value {
