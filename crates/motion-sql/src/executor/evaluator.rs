    ...
    match name.to_lowercase().as_str() {
        ...
        "vector_distance" => {
            ...
            let dist = match metric_str.to_lowercase().as_str() {
                "l2" | "euclidean" => motion_hnsw::distance::l2_distance(vec_a, vec_b),
                "cosine" => motion_hnsw::distance::cosine_distance(vec_a, vec_b),
                "inner_product" | "ip" => {
                    motion_hnsw::distance::inner_product_distance(vec_a, vec_b)
                }
                "manhattan" | "l1" => motion_hnsw::distance::manhattan_distance(vec_a, vec_b),
                _ => {
                    return Err(EvalError::InvalidArgument(format!(
        ...
        "l2_distance" => {
            ...
            Ok(Value::Double(motion_hnsw::distance::l2_distance(a, b) as f64))
        }
        ...
        "cosine_distance" => {
            ...
            let (a, b) = extract_vector_pair(&args[0], &args[1])?;
            Ok(Value::Double(
                motion_hnsw::distance::cosine_distance(a, b) as f64
            ))
        }
        ...
        "cosine_similarity" => {
            ...
            let (a, b) = extract_vector_pair(&args[0], &args[1])?;
            Ok(Value::Double(
                motion_hnsw::distance::cosine_similarity(a, b) as f64
            ))
        }
        ...
        "inner_product" => {
            ...
            let (a, b) = extract_vector_pair(&args[0], &args[1])?;
            Ok(Value::Double(motion_hnsw::distance::dot_product(a, b) as f64))
        }
