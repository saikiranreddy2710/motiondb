...
use motion_common::types::Key;
use motion_storage::sagetree::{CursorEntry, KeyRange, SageTree};
...
impl TableStore {
    ...
    pub fn get(&self, key_values: &[Value]) -> StorageResult<Option<Row>> {
        ...
        match self.hybrid.get(key.as_ref())? {
            Some(value_bytes) => {
                let sv = motion_common::types::Value::from_bytes(&value_bytes);
        ...
        let result = match range {
            ...
            ScanRange::Range { start, end } => {
                ...
                match (store.build_key(&start), store.build_key(&end)) {
                    ...
                    (Err(e), _) | (_, Err(e)) => Err(
                        motion_storage::sagetree::SageTreeError::structure_error(&e.to_string()),
                    ),
                }
            }
            ScanRange::Prefix(prefix) => match store.build_key(&prefix) {
                Ok(prefix_key) => store.hybrid.tree().scan_prefix(&prefix_key),
                Err(e) => Err(motion_storage::sagetree::SageTreeError::structure_error(
                    &e.to_string(),
                )),
            },
        };
